...
pub fn link_cef() -> Result<()> {
    ...
    {
        // Copy the CEF framework to the target directory
        copy_cef_macos()?;
...
/// Copy the CEF files to the target directory on macOS.
#[allow(dead_code)]
fn copy_cef_macos() -> Result<()> {
    let profile = var("PROFILE")?;
    let src = get_cef_cef_dir()?;
    let dst = get_cef_target_dir(&profile)?;
    ...
    // Set the correct @executable_path for the framework
    let framework_path =
        dst.join("Chromium Embedded Framework.framework/Chromium Embedded Framework");
    if framework_path.exists() {
        let output = std::process::Command::new("install_name_tool")
            .arg("-change")
            .arg("@executable_path/../Frameworks/Chromium Embedded Framework.framework/Chromium Embedded Framework")
            .arg("@executable_path/Chromium Embedded Framework.framework/Chromium Embedded Framework")
            .arg(framework_path)
            .output()?;

        if !output.status.success() {
            anyhow::bail!(
                "Failed to update framework path: {}",
